...
const ToggleUserAcc = (props) => {
    ...
    const admAPI = axios.get(USER_URL,
      {
        ...
        .then(res=>{
          ...
          res.data.data.map((data)=>{
            if(props.user.includes(data.id))
              {
                setPartner(data);
                //execute only once?
                console.log(data)
  ...
  const handleChange=()=> {
    ...
    const patchAPI = axios.patch(`${BASE_URL}/api/employee/users/${props.user}/status/${operate?'deactivate':'activate'}`,
      null,
      {
        headers:{'Authorization':AuthToken}
      })
